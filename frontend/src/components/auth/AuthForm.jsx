...
export default function AuthForm({ onSuccess }) {
  ...
  function validate() {
    ...
    if (!isValidUsername(username)) {
      return "שם משתמש חייב להכיל לפחות 2 אותיות באנגלית בלבד (ללא ספרות או תווים מיוחדים).";
    }

    // בדיקת סיסמה
    if (!isValidPassword(password)) {
      return "סיסמה חייבת להיות באורך 3–8 תווים, להכיל לפחות אות אחת ולפחות ספרה אחת (אותיות וספרות בלבד)";
    }

    // בדיקות נוספות רק בהרשמה
    if (isRegister) {
      if (!isValidFullName(fullName)) {
        return "יש להזין שם מלא תקין (לפחות 2 תווים ושיכיל אותיות).";
      }

      if (!isValidPhone(phone)) {
        return "מספר טלפון לא תקין. יש להזין מספר נייד ישראלי המתחיל ב־05 ובאורך 10 ספרות.";
      }

      if (!isValidEmail(email)) {
        return "אימייל לא תקין. יש להזין כתובת אימייל בפורמט נכון.";
      }

      if (password !== confirmPassword) {
        return "אימות סיסמה לא תואם לסיסמה.";
      }
    }
